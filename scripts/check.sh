...
# NOTE: use PASS=$((PASS+1)) instead of ((PASS++)) — the latter evaluates
# to exit code 1 when the result is 0, which kills the script under set -e.
...
for f in metadata.json contents/ui/main.qml contents/config/main.xml \
    ...
    if [[ -f "${PACKAGE_DIR}/${f}" ]]; then ok "${f} exists"
...
if [[ -z "${BACKUPS}" ]]; then ok "No backup files in package/"
else fail "Backup files found:\n${BACKUPS}"; fi

# 3. metadata.json has required fields
META="${PACKAGE_DIR}/metadata.json"
for field in Id Version Name KPackageStructure; do
    if grep -q "${field}" "${META}"; then ok "metadata.json has ${field}"
    else fail "metadata.json missing ${field}"; fi
done

# 4. main.xml: no openOnRowClick
MAIN_XML="${PACKAGE_DIR}/contents/config/main.xml"
if grep -q 'openOnRowClick' "${MAIN_XML}"; then
    fail "main.xml still contains openOnRowClick (removed in beta2)"
else
...
if [[ -f "${EDGE_QML}" ]]; then
    if grep -q 'Canvas' "${EDGE_QML}" && grep -q '_fwH' "${EDGE_QML}" && grep -q '_fwV' "${EDGE_QML}"; then
        ...
        fail "EdgeFadeBackground.qml missing Canvas fade implementation (_fwH/_fwV)"
    fi
else
...
if [[ -f "${PLASMOID}" ]]; then ok "${PLASMOID##*/} created"
else fail "${PLASMOID##*/} not found"; fi

# 11. No backup files inside the .plasmoid archive
if [[ -f "${PLASMOID}" ]]; then
    BAD="$(unzip -l "${PLASMOID}" | grep -E '\.backup|\.bak' || true)"
    if [[ -z "${BAD}" ]]; then ok "No backup files inside .plasmoid"
    else fail "Backup files inside .plasmoid:\n${BAD}"; fi
fi
